"""
RIT Grad Lab: In order to support Gaussian Parzen density estimator, we have first copied the code from this pull request:
https://github.com/scikit-learn/scikit-learn/pull/4515
and added Parzen option
...
with warnings.catch_warnings():
    ...
    from sklearn.utils import extmath, check_random_state, gen_batches
...
def mean_shift(X, bandwidth=None, seeds=None, bin_seeding=False,
               ...
               kernel='flat', gamma=1.0, nbr_weights=None):
    ...
    for my_mean in seeds:
        ...
        while True:
            ...
            my_mean = _kernel_update(my_old_mean, points_within, bandwidth,
                                     kernel, gamma, i_nbrs, nbr_weights)
...
def _kernel_update(old_cluster_center, points, bandwidth, kernel, gamma, i_nbrs=None, nbr_weights=None):
    ...
    implemented_kernels = ['flat', 'rbf', 'epanechnikov', 'biweight', 'parzen']
    ...
    if kernel == 'rbf':
        compute_weights = lambda p, b: np.exp(-1 * gamma *
                                              (p ** 2 / b ** 2))
    if kernel == 'parzen':
        compute_weights = lambda p, b: nbr_weights[i_nbrs][:, None] * np.exp(-1 * (p ** 2 / b ** 2))
...
class MeanShift(BaseEstimator, ClusterMixin):
    ...
    def fit(self, X, nbr_weights=None):
        """Perform clustering.
